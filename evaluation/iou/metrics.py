...
from trimesh.exchange.binvox import Binvoxer, load_binvox

BINVOX_PATH = '/home/liyh/code/TIMR/binvox'

def voxelize_mesh(mesh, binvox_path=BINVOX_PATH, export_type="off", **binvoxer_kwargs):
    """
    Custom voxelize_mesh that runs binvox in the temp directory with relative paths
    to avoid binvox's path handling bugs.
    """
    from trimesh.base import Trimesh
    if not isinstance(mesh, Trimesh):
        raise ValueError(f"mesh must be Trimesh instance, got {mesh!s}")
    
    binvoxer = Binvoxer(binvox_path=binvox_path, **binvoxer_kwargs)
    if binvoxer.file_type != "binvox":
        raise ValueError('Only "binvox" binvoxer `file_type` currently supported')
    
    with TemporaryDirectory() as folder:
        model_filename = f"model.{export_type}"
        model_path = os.path.join(folder, model_filename)
        with open(model_path, "wb") as fp:
            mesh.export(fp, file_type=export_type)
        
        # Run binvox from within the temp directory using relative path
        args = binvoxer._args.copy()
        args[0] = binvox_path  # Ensure absolute path to binvox
        args[-1] = model_filename  # Use relative filename
        
        # Run in the temp directory
        subprocess.check_output(args, cwd=folder, stderr=subprocess.STDOUT)
        
        out_path = os.path.join(folder, f"model.{binvoxer.file_type}")
...
def compute_mesh_iou(voxel1, voxel2, voxel_size=0.047):
    """
    Compute IoU between two voxelized meshes.
    Uses KDTree-based matching with tolerance to handle small misalignments.
    """
    ...
    # Get all voxel points in world coordinates
    v1_points = voxel1_surface.points
    v2_points = voxel2_surface.points
    
    # Use voxel scale as distance tolerance
    scale1 = voxel1_surface.scale[0] if hasattr(voxel1_surface.scale, '__len__') else voxel1_surface.scale
    scale2 = voxel2_surface.scale[0] if hasattr(voxel2_surface.scale, '__len__') else voxel2_surface.scale
    tolerance = (scale1 + scale2) / 2  # Allow 1 voxel tolerance
    
    # Build KD-trees
    tree1 = cKDTree(v1_points)
    tree2 = cKDTree(v2_points)
    
    # Count v1 points that have a v2 neighbor within tolerance
    dists_1_to_2, _ = tree2.query(v1_points)
    v1_in_v2 = np.sum(dists_1_to_2 <= tolerance)
    
    # Count v2 points that have a v1 neighbor within tolerance
    dists_2_to_1, _ = tree1.query(v2_points)
    v2_in_v1 = np.sum(dists_2_to_1 <= tolerance)
    
    # Compute IoU using the formula from original code
    n1 = len(v1_points)
    n2 = len(v2_points)
