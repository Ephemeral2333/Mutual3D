...
# global log files, set by eval.py before evaluation
_step_log_file = None  # per-step detail log
_result_log_file = None  # per-class result log (same as final txt)

def _log(msg):
    print(msg)
    if _step_log_file is not None:
        print(msg, file=_step_log_file, flush=True)

def _log_result(msg):
    print(msg)
    if _result_log_file is not None:
        print(msg, file=_result_log_file, flush=True)
...
CAD_labels = ['table', 'chair', 'bookshelf', 'sofa', 'trash_bin', 'cabinet', 'display', 'bathtub']  # 原始8类
# CAD_labels = ['table', 'chair', 'display', 'cabinet', 'trash_bin']  # 暂时只评估5类
...
def eval_det_cls_w_mesh(pred, gt, ovthresh, use_07_metric):
    ...
    for d in range(nd):
        #if d % 100 == 0: print(d)
        _log(f'[eval mesh] {d + 1} / {nd} from {scan_ids[d]}')
        ...
        if len(points_gt) > 0:
            ...
            for j in range(len(points_gt)):
                ...
                if (mesh_pred.faces.shape[0] > 10000):
                    _log(f'[WARN] mesh_pred very large {mesh_pred.vertices.shape}, {mesh_pred.faces.shape}, simplifying...')
                    import open3d as o3d
                    mesh_o3d = o3d.geometry.TriangleMesh()
                    mesh_o3d.vertices = o3d.utility.Vector3dVector(mesh_pred.vertices)
                    mesh_o3d.triangles = o3d.utility.Vector3iVector(mesh_pred.faces)
                ...
                # clean degenerate triangles before BVH construction
                mesh_pred_clean = trimesh.Trimesh(vertices=mesh_pred.vertices, faces=mesh_pred.faces, process=True)
                # _log(f'[eval mesh] gt_j={j} mesh v={mesh_pred_clean.vertices.shape[0]} f={mesh_pred_clean.faces.shape[0]} gt_pts={points_gt[j].shape[0]}')
                ...
                try:
                    ...
                    free_bytes = (soft - mem_bytes) if soft != resource.RLIM_INFINITY else float('inf')
                    if free_bytes < 10 * 1024 ** 3:  # 剩余虚拟内存 < 10GB 时跳过
                        _log(f'[SKIP] low memory ({free_bytes/1024**3:.1f}GB free), skipping closest_point')
                        pcr_mesh = 0.0
                    else:
                        closest, distance, _ = trimesh.proximity.closest_point(mesh_pred_clean, points_gt[j])
                        pcr_mesh = (distance < 0.047).sum() / len(distance)
                except Exception as e:
                    _log(f'[SKIP] closest_point failed: {e}')
                    pcr_mesh = 0.0
            ...
            _log(f'[eval mesh] best mesh PCR {ovmax_mesh}, from {jmax_mesh} in {j}')

        if ovmax_mesh > ovthresh:
            tp_mesh[d] = 1.
            tp_mesh_iou[d] = ovmax_mesh
            R['det_mesh'][jmax_mesh] = 1
            _log(f'[eval mesh] mesh TP ++')
        else:
            fp_mesh[d] = 1.
        
        # 释放内存：删除当前 mesh 的引用并强制垃圾回收
        meshes[d] = None
        del mesh_pred
        if d % 1 == 0:
            gc.collect()
    ...
    _log(f'[eval mesh] tp = {tp_mesh}, fp = {fp_mesh}, fn = {fn_mesh}, npos = {npos}')
...
def eval_det_multiprocessing_w_mesh(pred_all, gt_all, ovthresh, use_07_metric):
    ...
    for classname in sorted(gt.keys()):
        ...
        _log(f'[eval mesh] class {CAD_labels[classname]}')
        ...
        # write per-class result immediately
        (rec_, prec_, ap_), (PQ_, SQ_, RQ_) = ret_value
        clsname = CAD_labels[classname]
        _log_result(f'[class {clsname}] AP={ap_:.4f} Prec={prec_:.4f} Rec={rec_:.4f} PQ={PQ_:.4f} SQ={SQ_:.4f} RQ={RQ_:.4f}')
