...
def export(mesh_file, agg_file, seg_file, meta_file, label_map):
    ...
    try:
        ...
        if len(mesh.triangles) == 0:
            print(f'[WARN] No triangles found in mesh file: {mesh_file}')
            superpoints = np.zeros(len(mesh_vertices), dtype=np.int32)
        else:
    ...
    label_ids = np.zeros(shape=(num_verts), dtype=np.uint8)
    ...
    instance_ids = np.zeros(shape=(num_verts), dtype=np.uint8)
    ...
    return mesh_vertices, label_ids, instance_ids, instance_bboxes, superpoints, object_id_to_label_id
...
def generate(scan2cad_annotation):
    ...
    meta_file = os.path.join(path_config.metadata_root, 'scans', scene_name, scene_name + '.txt')
    lines = open(meta_file).readlines()
    for line in lines:
        if 'axisAlignment' in line:
            axis_align_matrix = [float(x) \
    ...
    mesh_vertices, semantic_labels, instance_labels, instance_bboxes, superpoints, _ = export(mesh_file, agg_file, seg_file, meta_file, RAW2RFS)

    instance_relabels = np.zeros_like(instance_labels)
    instance_rebboxes = []
    ...
    for i in range(len(instance_bboxes)):
        ...
        if bbox[6] == 255: continue

        cad_lbl = RFS2CAD[bbox[6]]
    ...
    np.savez(output_file, mesh_vertices=mesh_vertices, semantic_labels=semantic_labels, instance_labels=instance_relabels, superpoints=superpoints)
